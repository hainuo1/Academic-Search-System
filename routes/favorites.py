...
def favorites():
    ...
    cursor.execute("SELECT COUNT(*) FROM Favorites WHERE UserID = %s", (user_id,))
...
def toggle_favorite():
    ...
    cursor.execute("SELECT DocumentID FROM Documents WHERE DocumentID = %s", (doc_id,))
    ...
    if existing:
        cursor.execute("DELETE FROM Favorites WHERE FavoriteID = %s", (existing[0],))
        is_favorited = False
    else:
        cursor.execute("INSERT INTO Favorites (UserID, DocumentID) VALUES (%s, %s)", (user_id, doc_id))
        is_favorited = True
