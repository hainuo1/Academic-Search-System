...
def search_citation():
    ...
    cursor.execute("""
        SELECT TOP 20 DocumentID, Title, Author
        FROM Documents
        WHERE Title LIKE ?
        AND DocumentID <> ?
...
def save_citation(doc_id):
    ...
    cursor.execute("""
        SELECT UploadUserID FROM Documents WHERE DocumentID = ?
    """, (doc_id,))
    doc = cursor.fetchone()
    ...
    if target_ids:
        ...
        placeholders = ','.join('?' for _ in target_ids_int)
    ...
    try:
        cursor.execute("""
            DELETE FROM Citation WHERE SourceDocumentID = ?
        """, (doc_id,))
