...
def stats():
    ...
    # 热门关键词排行（TOP 10）
    cursor.execute("""
        SELECT Keyword AS SearchKeyword, SearchCount AS cnt
    ...
    # 活跃用户排行（TOP 10）
    cursor.execute("""
        SELECT u.UserName, usc.SearchCount AS cnt
    ...
    # 高被引文献排行（TOP 10）
    cursor.execute("""
        SELECT d.DocumentID, d.Title, COUNT(*) AS cnt
        FROM Citation c
    ...
    # 热门文献（按浏览次数）
    cursor.execute("""
        SELECT DocumentID, Title, ViewCount
        FROM Documents
    ...
    # 热门文献（按下载次数）
    cursor.execute("""
        SELECT DocumentID, Title, DownloadCount
        FROM Documents
    ...
    # 热门收藏文献排行
    cursor.execute("""
        SELECT d.DocumentID, d.Title, COUNT(f.FavoriteID) AS cnt
        FROM Favorites f
