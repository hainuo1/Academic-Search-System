  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-100 shadow-sm p-4 text-center">
        <div class="text-2xl font-bold text-slate-800">{{ fmtWind(info.max_wind) }} <span class="text-sm text-slate-400">kts</span></div>
...
const minPressureDisplay = computed(() => {
  ...
  if (obs.length === 0) return info.min_pressure ? fmtPressure(info.min_pressure) : '-'
...
async function fetchData(){try{let[a,b]=await Promise.all([api.get(`/api/typhoon/${typhoonId}`),api.get(`/api/typhoon/${typhoonId}/track`)]); if(a.data.code===200) Object.assign(info,a.data.data); if(b.data.code===200) trackPoints.value=b.data.data.points}catch(e){console.error(e)}}
