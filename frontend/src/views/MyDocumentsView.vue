...
onMounted(async () => {
  try { const r = await api.get('/api/my_documents'); if (r.data.code === 200) docs.value = r.data.data.documents } catch (e) {}
})
...
function downloadPdf(docId) {
  fetch(`http://localhost:5000/api/download/${docId}/check`, {
    ...
    link.href = `http://localhost:5000/api/download/${docId}?token=${encodeURIComponent(token || '')}`
