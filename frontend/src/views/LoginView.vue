...
async function go(){if(!un.value||!pw.value){err.value='用户名和密码不能为空';return};ld.value=true;err.value='';try{const r=await api.post('/api/login',{username:un.value,password:pw.value});if(r.data.code===200){auth.login(r.data.data.token,r.data.data.username,r.data.data.user_id);flash.show('登录成功！','success');router.push('/dashboard')}else err.value=r.data.message}catch(e){err.value=e.response?.data?.detail||e.response?.data?.message||'网络错误'}finally{ld.value=false}}
</script>