...
async function load(p) { try { const r = await api.get('/api/favorites', { params: { page: p } }); if (r.data.code === 200) { docs.value = r.data.data.documents; pg.value = r.data.data.page; tps.value = r.data.data.total_pages; totalCount.value = r.data.data.total_count } } catch (e) {} }
async function rm(d) { try { const r = await api.post('/api/toggle_favorite', { document_id: d.id }); if (r.data.code === 200 && !r.data.data.is_favorited) { docs.value = docs.value.filter(x => x.id !== d.id); totalCount.value-- } } catch (e) {} }
function downloadPdf(docId) {
  fetch(`http://localhost:5000/api/download/${docId}/check`, {
    headers: { 'Authorization': `Bearer ${localStorage.getItem('token') || ''}` }
  }).then(r => r.json()).then(data => {
    if (data.code !== 200) { alert(data.message || '文件不存在'); return }
    const t = localStorage.getItem('token'); const a = document.createElement('a')
    a.href = `http://localhost:5000/api/download/${docId}?token=${encodeURIComponent(t || '')}`
    document.body.appendChild(a); a.click(); document.body.removeChild(a)
  }).catch(() => { alert('文件不存在，可能已被删除') })
}
onMounted(() => load(1))
</script>
