      ...
      <div v-for="t in tornadoes" :key="t.event_id"
        ...
        <div class="flex items-start justify-between mb-3">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-slate-400 mt-0.5">{{ t.datetime }} · {{ t.province || '未知' }}</p>
          </div>
        ...
        <div class="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-slate-100">
          <div class="text-center">
            <div class="text-lg font-bold text-red-600">{{ t.deaths || 0 }}</div>
            <div class="text-[10px] text-slate-400">死亡</div>
          </div>
          <div class="text-center">
            <div class="text-lg font-bold text-amber-600">{{ t.injuries || 0 }}</div>
            <div class="text-[10px] text-slate-400">受伤</div>
          </div>
          <div class="text-center">
            <div class="text-lg font-bold text-slate-700">{{ t.casualties || 0 }}</div>
...
function efBadgeClass(ef) {
  const m = parseInt(String(ef || '').replace('EF', '')) || 0
