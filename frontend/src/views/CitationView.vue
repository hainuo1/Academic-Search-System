...
async function searchCit() { if (!skw.value.trim()) return; searched.value = true; try { const r = await api.get('/api/search_citation', { params: { keyword: skw.value.trim(), doc_id: route.params.id } }); if (r.data.code === 200) sres.value = r.data.data.results } catch (e) {} }
