...
const un = ref('')
const cap = ref('')
const capUrl = ref(`${api.defaults.baseURL}/api/captcha?t=${Date.now()}`)
const q1 = ref('')
const q2 = ref('')
const a1 = ref('')
const a2 = ref('')
...
function refreshCap() { capUrl.value = `${api.defaults.baseURL}/api/captcha?t=${Date.now()}` }
...
async function goStep2() {
  err.value = ''
  if (np.value !== cp.value) { err.value = '两次输入的密码不一致'; return }
  if (np.value.length < 8) { err.value = '密码至少需要8个字符'; return }
  ld.value = true
  try {
    const r = await api.post('/api/forgot_password', {
