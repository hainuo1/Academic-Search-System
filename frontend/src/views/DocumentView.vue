...
async function toggleFav() {
  try { const r = await api.post('/api/toggle_favorite', { document_id: route.params.id }); if (r.data.code === 200) fav.value = r.data.data.is_favorited } catch (e) { console.error('收藏切换失败:', e) }
}

async function downloadPdf() {
  dlErr.value = ''
  try {
    const resp = await api.get(`/api/download/${route.params.id}`, { responseType: 'blob' })
